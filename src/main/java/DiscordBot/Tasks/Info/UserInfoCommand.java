...
public class UserInfoCommand {
    ...
    public static void getCommand(MessageReceivedEvent event)
    {
        ...
        if (content.indexOf(SetPrefixCommand.getPrefix(event.getGuild().getIdLong()) + "userinfo") == 0)
        {
            User toGetInfo = event.getJDA().getUsersByName(content.substring(SetPrefixCommand.getPrefix(event.getGuild().getIdLong()).length() + 9), true).get(0);
